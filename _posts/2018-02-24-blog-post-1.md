...
So let's examine this a little bit more. There are a few interesting constructs that allow us to do that first bullet point. Jinja2 components are within `{% raw %}{% %}{% endraw %}` or within `{{ }}`. We can see a check for a variable called `title` with `{% if title %}`. If we want to access the value of that variable, it's as easy as `{{ title }}`. Be sure to end this with an `{% endif %}`.
